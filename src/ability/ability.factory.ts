...
export class AbilityFactory {
    defineAbility(user: User) {
        ...
        if (user.role.role !== 'ADMIN') {
            ...
            cannot(Action.CreateFarm, User).because('Only admins can create farms');
            cannot(Action.ReadFarm, User).because('Only admins can read farms');
            cannot(Action.UpdateFarm, User).because('Only admins can update farms');
            cannot(Action.DeleteFarm, User).because('Only admins can delete farms');
        }
        return build({
