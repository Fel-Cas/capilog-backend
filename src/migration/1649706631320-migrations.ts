...
export class migrations1649706631320 implements MigrationInterface {
    name = 'migrations1649706631320'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`order\` (\`idOrder\` int NOT NULL AUTO_INCREMENT, \`startDate\` timestamp NOT NULL, \`description\` varchar(255) NOT NULL, \`arriveDate\` timestamp NULL, \`exitDate\` timestamp NULL, \`destinationArriveDate\` timestamp NULL, \`destintionExitDate\` timestamp NULL, \`isBill\` tinyint NULL DEFAULT 0, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`first_farm\` int NULL, \`last_farm\` int NULL, \`order_statement\` int NULL, \`user\` varchar(255) NULL, \`truck\` varchar(255) NULL, PRIMARY KEY (\`idOrder\`)) ENGINE=InnoDB`);
